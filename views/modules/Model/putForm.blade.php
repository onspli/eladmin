...
@section('modal-footer')
  ...
  <button type="button" class="btn btn-danger mr-3" data-elaaction="delRow" data-elaarg{{$elaModule->getKeyName()}}="{{$row->getKey()}}" data-eladone="$('#dynamic .modal').modal('hide');redrawCrudTable();" data-confirm="Opravdu smazat?"><i class="fas fa-trash-alt"></i> Smazat</button>
  @endif
  <button type="button" class="btn btn-secondary" data-dismiss="modal"><i class="far fa-times-circle"></i> Zpět</button>
  @if($eladmin->auth('putRow'))
    <button type="submit" form="modal-form" class="btn btn-primary"><i class="fas fa-save"></i> Upravit</button>
  @endif
@endsection
